...
procedure menu (var maestro:archMaestro; var det1,det2:archDetalle);
	...
	procedure realizarBajas(var mae:archMaestro);
	
		procedure leerMaestro(var mae:archMaestro;var regMae:provincia);
		begin
			if(not EOF(mae))then begin
				read(mae,regMae);
			end
			else
				regMae.nombre:=VALOR_ALTO;
		end;
	...
	var
		...
		pos:integer;
	begin
		write('ingrese la provincia a eliminar: '); readln(prov);
		reset(mae);
		leerMaestro(mae, regMae);
		while(regMae.nombre <> VALOR_ALTO) and (regMae.nombre <> prov)do begin
			leerMaestro(mae, regMae);
		end;
		if(regMae.nombre <> VALOR_ALTO) then begin
			pos:=filePos(mae) - 1;
			seek(mae, filesize(mae)-1);
			read(mae, aux); //guardo el registro de la ultima posicion
			seek(mae, pos); //vas a la posicion a eliminar
			write(mae, aux);//sobreescribo la posicion
			seek(mae, filesize(mae)-1); //voy al anteultimo elemento
			truncate(mae); //elimino el ultimo registro
		end
		else
			writeln('no se encontro el elemento a borrar');
		close(mae);
	end;
